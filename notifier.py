"""
NTFY notifier for the execution bot.
Sends entry and exit notifications in English, no emojis.
"""
...
def _send(message: str, title: str, priority: str = '3',
          tags: str = 'robot') -> bool:
    try:
        headers = {
            'Title': title,
...
def notify_entry(symbol: str, side: str, entry_price: float,
    ...
    return _send('\n'.join(lines), f"{mode} {direction} {symbol}",
                 priority, 'heavy_check_mark' if side == 'long' else 'arrow_down')
...
def notify_exit(symbol: str, side: str, entry_price: float,
    ...
    return _send('\n'.join(lines), f"{mode} {status} {symbol}",
                 priority, tag)
